...
Run from the debateai/ directory:
...
from backend.debate.debate_state import DebateResult, DebateState
from backend.utils.exceptions import DebateAIError
...
def _print_banner() -> None:
    ...
    console.print(Panel(
        ...
        subtitle="[dim]Version 2 — Adaptive Strategy Engine[/dim]",
        box=box.DOUBLE_EDGE,
        border_style="cyan",
    ))
    console.print()
...
def _print_consensus(result: DebateResult) -> None:
    ...
    # Timing summary from structured message log
    state: DebateState = result.state
...
async def run_debate(question: str, settings: Settings) -> None:
    ...
    # Only print opening/rebuttal panels when the strategy produced them.
